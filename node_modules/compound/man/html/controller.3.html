...
<body id='manpage'>
  ...
  <div class='man-navigation' style='display:none'>
    ...
    <a href="#OUTPUT-METHODS">OUTPUT METHODS</a>
    <a href="#FLOW-CONTROL-METHODS">FLOW CONTROL METHODS</a>
...
<p>Controller context has the following set of members:</p>
...
<h3 id="render-view-params-">render([view[, params]])</h3>
...
<h3 id="renderView-view-callback-">renderView(view[, callback])</h3>

<p>Render view and return result to callback(err, html) or send to client when
callback is missing. This method does not accept params and works with
<code>viewContext</code> member of controller. Run <code>prepareViewContext</code> to create that
member.</p>

<h3 id="send-smth-">send(smth)</h3>

<p>Send text, status code or json object to client</p>
...
<h3 id="redirect-location-">redirect(location)</h3>

<p>Redirect client to specific location</p>
...
<p>Send header to client</p>

<h3 id="flash-type-message-">flash(type, message)</h3>

<p>Display flash message</p>
...
<p>This <code>create</code> action sends a flash info on success and a flash error on fail.</p>

<h2 id="FLOW-CONTROL-METHODS">FLOW CONTROL METHODS</h2>
...
<h3 id="before-name-hook-params-">before([name, ]hook[, params])</h3>

<p>Invoke <code>hook</code> before any action. Name param is optional when hook is named
...
pass control to the next function in the chain.</p>

<h3 id="after-name-hook-params-">after([name, ]hook[, params])</h3>

<p>Invoke <code>hook</code> after any action. Name param is optional when hook is named
function. Params object is the same as for before hook.</p>

<h3 id="skipBefore-name-params-">skipBefore(name, params)</h3>

<p>Skip before hook by it's name. Params object allows to specify skip/only
actions.</p>

<h3 id="skipAfter-name-params-">skipAfter(name, params)</h3>

<p>Skip after hook by it's name. Params object allows to specify skip/only
actions.</p>

<h3 id="next-err-">next(err)</h3>

<p>Go to next hook/action in chain. When error param passed to next rest of chain
skipped and error passed to error handling middleware.</p>
